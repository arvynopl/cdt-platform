...
def main() -> None:
    ...
    rho = evaluate.spearman_recovery(disp_records, "disposition")
    precision, recall = evaluate.presence_precision_recall(records, "disposition")
    exact, within, catastrophic = evaluate.severity_accuracy(disp_records, "disposition")
    cm = evaluate.confusion_matrix(disp_records, "disposition")
    ...
    import statistics
    for sev in SEVERITIES:
        vals = [abs(r.dei) for r in disp_records if r.injected_severity == sev]
